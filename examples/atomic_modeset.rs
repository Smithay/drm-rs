...
pub fn main() {
    ...
    let (better_planes, compatible_planes): (Vec<control::plane::Handle>, Vec<control::plane::Handle>) =
        planes.planes()
            .iter()
            .map(|x| *x)
            .filter(|plane| {
                card.get_plane(*plane).map(|plane_info| {
    ...
    card.atomic_commit(&[AtomicCommitFlags::AllowModeset], atomic_req).expect("Failed to set mode");
